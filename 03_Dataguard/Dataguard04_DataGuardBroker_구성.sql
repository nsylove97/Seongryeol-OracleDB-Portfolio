...
================================================================================
 ...
 목차
   1. Broker 활성화 전 사전 준비
      1-1. SPFILE 사용 여부 확인
      1-2. LOG_ARCHIVE_DEST_2 비우기 (VM1, VM3)
   2. DG_BROKER_START 활성화
      2-1. DG_BROKER_START=TRUE 설정 (VM1, VM3)
      2-2. DMON 프로세스 기동 확인
   3. DGMGRL — Configuration 생성 및 활성화
      3-1. DGMGRL 접속
      3-2. Configuration 생성
      3-3. Standby DB 추가
      3-4. Configuration 활성화
      3-5. Broker Configuration File 생성 확인
   4. 구성 상태 확인
      4-1. SHOW CONFIGURATION
      4-2. SHOW DATABASE
      4-3. SHOW DATABASE VERBOSE
   5. Protection Mode 확인
      5-1. DGMGRL에서 확인
      5-2. SQL*Plus에서 확인
      5-3. Protection Mode 변경 방법 (참고)
   6. VALIDATE DATABASE — 구성 검증
      6-1. VALIDATE DATABASE
      6-2. VALIDATE STATIC CONNECT IDENTIFIER
      6-3. StaticConnectIdentifier 수정 (.localdomain 불일치 해결)
      6-4. VALIDATE NETWORK CONFIGURATION
   7. 관련 뷰 & 파일 정리
      7-1. v$dataguard_config
      7-2. v$dg_broker_config
      7-3. Broker Log 파일 위치
================================================================================
*/
...
/*
   ...
   (DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.111.50)(PORT=1521))
   (CONNECT_DATA=(SERVICE_NAME=orcl_DGMGRL)...))
   ORA-12514: TNS:listener does not currently know of service requested
   Failed.

   ※ 원인: Broker가 사용하는 SERVICE_NAME이 'orcl_DGMGRL'인데
           listener.ora의 GLOBAL_DBNAME은 'orcl_DGMGRL.localdomain'으로 등록됨
           → 도메인 불일치로 리스너가 서비스를 찾지 못함
*/
...
/* --------------------------------------------------------------------------
   7-1. v$dataguard_config — DG 구성원 목록
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   7-2. v$dg_broker_config — Broker 구성원 및 접속 식별자 확인
   -------------------------------------------------------------------------- */
...
/* ============================================================================
   ...
   ORA-12514 원인               GLOBAL_DBNAME(.localdomain)과
                                Broker StaticConnectIdentifier 도메인 불일치
